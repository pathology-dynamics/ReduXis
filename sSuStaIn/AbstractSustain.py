    ...
    def run_sustain_algorithm(self, plot=False, plot_format="png", **kwargs):
        ...
        for s in range(self.N_S_max):
            ...
            if pickle_filepath.exists():
                ...
                print("Failed to find pickle file: " + pickle_filename_s + ". Running SuStaIn model for " + str(s) + " subtype" + ("s" if s > 0 else "") + ".")
                ...
                ml_sequence_mat_EM, \
                ml_f_mat_EM,        \
                ml_likelihood_mat_EM        = self._estimate_ml_sustain_model_nplus1_clusters(self.__sustainData, ml_sequence_prev_EM, ml_f_prev_EM)
                ...
                samples_sequence,   \
                samples_f,          \
                samples_likelihood          = self._estimate_uncertainty_sustain_model(self.__sustainData, seq_init, f_init)
            ...
            prob_subtype,           \
            prob_stage,             \
            prob_subtype_stage               = self.subtype_and_stage_individuals(self.__sustainData, samples_sequence, samples_f, N_samples)
        ...
        if "save_path" not in kwargs:
            ...
            if len(figs) > 1:
                ...
                for num_subtype, fig in zip(range(N_subtypes), figs):
                    ...
                    plot_fname = Path(
                        self.output_folder
                    ) / f"{self.dataset_name}_subtype{N_subtypes}_subtype{num_subtype+1}-separated_PVD_{N_folds}fold_CV.{plot_format}"
                    # Save the figure
            ...
            # Otherwise default single plot
                ...
                plot_fname = Path(
                    self.output_folder
                ) / f"{self.dataset_name}_subtype{N_subtypes}_PVD_{N_folds}fold_CV.{plot_format}"
                # Save the figure
                fig.savefig(plot_fname, bbox_inches='tight')
                fig.show()
        ...
        if N_S == 1:
            ...
            for ix_cluster_split in range(N_S - 1):
                ...
                if this_N_cluster > 1:
                    ...
                    print(' + Resolving', N_S - 1, 'cluster problem')
                    ...
                    print('- ML likelihood is', this_ml_likelihood[0].item())
                else:
                    print(f'Cluster {ix_cluster_split + 1} of {N_S - 1} too small for subdivision')
            print(f'Overall ML likelihood is', ml_likelihood.item())
